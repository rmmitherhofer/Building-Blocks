...
namespace SnapTrace.AspNetCore
{
    ...
    {
        public static SnapTrace.Http.HttpRequest Create(Microsoft.AspNetCore.Http.HttpRequest httpRequest)
        {
            //OBTEM OS DADOS DO REQUEST PARA COMEÇAR A MONTAR OS DADOS PARA ENVIAR PARA API

            if (httpRequest == null)
                throw new ArgumentNullException(nameof(httpRequest));

            Session session = SnapTrace.InternalHelpers.WrapInTryCatch(() => GetSession(httpRequest));
            session = session ?? new Session();

            bool isAuthenticated = httpRequest.HttpContext?.User?.Identity?.IsAuthenticated ?? false;

            SnapTrace.Http.HttpRequest result = new SnapTrace.Http.HttpRequest(new SnapTrace.Http.HttpRequest.CreateOptions
            {
            ...
            propertiesOptions.Cookies = SnapTrace.AspNetCore.InternalHelpers.ToKeyValuePair(httpRequest.Cookies);
            propertiesOptions.Headers = SnapTrace.AspNetCore.InternalHelpers.ToKeyValuePair(httpRequest.Headers);
            propertiesOptions.QueryString = SnapTrace.AspNetCore.InternalHelpers.ToKeyValuePair(httpRequest.Query);
            ...
            if (SnapTrace.InternalHelpers.CanReadRequestInputStream(propertiesOptions.Headers))
            {
                if (KissLogConfiguration.Options.Handlers.ShouldLogInputStream.Invoke(result) == true)
                {
                    propertiesOptions.InputStream = SnapTrace.InternalHelpers.WrapInTryCatch(() =>
                    {
