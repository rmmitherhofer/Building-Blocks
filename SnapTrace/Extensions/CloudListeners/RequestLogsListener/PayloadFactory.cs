...
using SnapTrace.RestClient.Requests.CreateRequestLog.Http;
using System.Net;
...
namespace SnapTrace.CloudListeners.RequestLogsListener
{
    internal static class PayloadFactory
    {
        public static CreateRequestLogRequest Create(FlushLogArgs args)
        {
            ...
            IEnumerable<SnapTrace.LogMessage> logMessages = args.MessagesGroups.SelectMany(p => p.Messages).OrderBy(p => p.DateTime).ToList();
        ...
        internal static HttpProperties Create(SnapTrace.Http.HttpProperties httpProperties)
        {
        ...
        internal static RequestProperties Create(SnapTrace.Http.RequestProperties requestProperties)
        {
        ...
        internal static ResponseProperties Create(SnapTrace.Http.HttpResponse httpResponse)
        {
        ...
        internal static SnapTrace.RestClient.Requests.CreateRequestLog.LogMessage Create(LogMessage message, DateTime startRequestDateTime)
        {
            if (message == null)
                throw new ArgumentNullException(nameof(message));

            var result = new SnapTrace.RestClient.Requests.CreateRequestLog.LogMessage
            {
        ...
        internal static SnapTrace.RestClient.Requests.CreateRequestLog.CapturedException Create(CapturedException exception)
        {
            if (exception == null)
                throw new ArgumentNullException(nameof(exception));

            return new SnapTrace.RestClient.Requests.CreateRequestLog.CapturedException
            {
        ...
        internal static User CreateUser(SnapTrace.Http.HttpRequest httpRequest)
        {
            if (httpRequest == null)
