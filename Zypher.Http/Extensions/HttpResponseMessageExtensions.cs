...
public static class HttpResponseMessageExtensions
{
    ...
    public static async Task<TResponse?> ReadAsAsync<TResponse>(this HttpResponseMessage response, JsonSerializerOptions? options = null)
    {
        ...
        return JsonExtensions.Deserialize<TResponse>(content, options);
    }
    ...
    /// <exception cref="CustomHttpRequestException">Thrown when the status code is Unauthorized or InternalServerError and throwException is true.</exception>
    public static bool HasErrors(this HttpResponseMessage response, bool throwException = true)
    {
        ...
        if (statusCode is HttpStatusCode.Unauthorized or HttpStatusCode.InternalServerError)
        {
            if (throwException)
            {
                ...
                throw new CustomHttpRequestException(statusCode, $"{method} - {uri} - {(int)statusCode} - {statusCode}");
            }
            return true;
        }
